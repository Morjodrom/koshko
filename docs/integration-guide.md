...
Cross-origin application messaging remains subject to the browser's normal security model. Continue to use explicit target origins and validate incoming messages for your application's own `postMessage` protocol. When Koshko has permission for a receiving frame, its Log records delivered messages as raw **PostMessage** entries in addition to any semantic Koshko entry. The Timeline places each raw entry in a lane for the receiving top-level page or iframe; it does not guess an uncertain sender frame. This observation does not validate or reconstruct the application's protocol.

The panel hides messages classified as browser-extension traffic by default so page and iframe messages remain the focus. Use the global **Extensions** checkbox to reveal retained extension messages in Timeline, Log, AI Log, and JSONL export. Classification uses editable payload signatures rather than an extension identity: Chrome does not identify the responsible extension on a page-level `MessageEvent`, page code can spoof a signature, and unknown extensions remain visible until you add a rule. Options includes editable React DevTools and PIXI DevTools defaults plus constrained controls for custom rules; **Reset all rules to defaults** replaces the complete rule list. Rule changes immediately reclassify already-captured entries.
...
2. In `chrome://extensions`, enable **Developer mode**, choose **Load unpacked**, and select:
   ...
   ```
3. Open the extension **Options** page. Add and grant the host origin and every iframe origin you will inspect. Review **Message filtering** if browser tooling emits its own page messages.
4. Reload the target tab, open DevTools, and select the **Koshko** panel.
5. Trigger the instrumented action. Check that the Timeline shows semantic source actors and receiving-frame lanes for raw page messages, and that directed semantic events show the intended target. Check the Log/details view for the event name, correlation ID, tags, and safe details. If the application emits state mutations, open **Global State** and verify the reconstructed JSON value. To ask an LLM for help, open **AI Log**, select a context budget, and use **Copy for AI**. The generated capsule stays local until you paste it elsewhere and may contain all captured diagnostic data, so review it before sharing.
